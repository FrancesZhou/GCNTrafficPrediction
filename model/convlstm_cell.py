...
class ConvLSTMCell(rnn_cell_impl.RNNCell):
  ...
  def call(self, inputs, state, scope=None):
    ...
    #
    cell, hidden = state
    new_hidden = _conv(args=[inputs, hidden], filter_size=self._kernel_shape,
...
def _conv(self, args, filter_size, num_features, bias, bias_start=0.0,
  ...
  if self.dy_adj>0:
      filter_local_expand = tf.reshape(tf.eye(total_arg_size_depth*filter_size[0]*filter_size[1], dtype=tf.float32),
                                       (total_arg_size_depth*filter_size[0]*filter_size[1], total_arg_size_depth, filter_size[0], filter_size[1]))
      ...
      input_local_all = tf.reshape(input_local_all, (-1, num_features, total_arg_size_depth*filter_size[0]*filter_size[1]))
      # input_local_all: [batch_size*row*col, output_channel, input_channel*s*s]
      #####
      kernel = vs.get_variable(
      ...
      if self.dy_filter>0:
          ...
          dy_gen_f = tf.nn.conv2d(dy_f, [filter_size[0], filter_size[1], f_input_dim, filter_size[0]*filter_size[1]], strides=strides, padding='SAME')
          reshape_dy_f = tf.reshape(dy_gen_f, (-1, filter_size[0], filter_size[1]))
      else:
          dy_f = tf.reshape(dy_f, (-1, self._input_shape[0], self._input_shape[1], filter_size[0], filter_size[1]))
      ...
      dy_kernel = tf.reshape(dy_kernel, (-1, filter_size[0], filter_size[1], num_features, total_arg_size_depth))
      dy_kernel = tf.transpose(dy_kernel, (0, 3, 4, 1, 2))
      dy_kernel = tf.reshape(dy_kernel, (-1, num_features, total_arg_size_depth*filter_size[0]*filter_size[1]))
