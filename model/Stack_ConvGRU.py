...
class Stack_ConvGRU():
    ...
    def build_easy_model(self):
        ...
        for i in range(self.num_layers):
            ...
            with tf.variable_scope('gconv_%d' % (i+1)):
                gconv_output = self._gconv(num_nodes=self.input_shape[0]*self.input_shape[1],
                                           inputs=x, dy_adj_mx=f, output_size=self.num_units,
        ...
        # GRU cell
        cell = tf.nn.rnn_cell.GRUCell(num_units=self.num_units, activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name='gru')
        #
