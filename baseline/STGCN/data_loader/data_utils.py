...
def cheb_poly_approx_tf(L, Ks, n):
    ...
    :param L: tensor, [n_route, n_route], graph Laplacian.
    ...
    if Ks > 1:
        ...
        raise ValueError('ERROR: the size of spatial kernel must be greater than 1, but received "{0}".'.format(Ks))
...
def seq_gen_2(data_seq, n_frame, n_route, channels):
    num = len(data_seq) - n_frame + 1
    tmp_seq = np.zeros((num, n_frame, n_route, channels))
    for i in range(num):
        tmp_seq[i] = data_seq[i: i+n_frame]
    return tmp_seq
