...
def build_model(trainY, testY, trainimage, testimage,
                feature_len,
                minMax,
                seq_len=8,
                batch_size=64,
    ...
    res = Dense(units=2, activation='sigmoid')(lstm_out)
    ...
    if trainable:
        model.fit(trainimage, trainY, batch_size=batch_size, epochs=max_epoch, validation_split=0.1,
                  callbacks=[earlyStopping, model_checkpoint])
    else:
        model.load_weights(fname_param)
    # testLoss = model.evaluate([testimage, testtopo], testY)
    score = model.evaluate(trainimage, trainY, batch_size=batch_size, verbose=0)
    ...
    score = model.evaluate(testimage, testY, batch_size=batch_size, verbose=0)
    ...
    prediction = model.predict(testimage, batch_size=batch_size, verbose=0)
